...
import { formatMoney, blendedTokenRate } from '../lib/estimator';
import { ProgressRing, SectionLabel } from '../components/common/Ledger';
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-white/[0.03]">
              {agents
                ...
                .sort((a, b) => b.stats.tokensUsed - a.stats.tokensUsed)
                .map(a => (
                  <tr key={a.id}>
                    ...
                    <td className="py-2.5 text-right font-mono text-xs text-gray-400 tabular-nums">
                      {a.stats.totalRuns.toLocaleString()}
                    </td>
                    <td className="py-2.5 text-right font-mono text-xs text-gray-400 tabular-nums">
                      {(a.stats.tokensUsed / 1_000_000).toFixed(2)}M
                    </td>
                    <td className="py-2.5 text-right font-mono text-xs text-gray-300 tabular-nums">
                      {formatMoney((a.stats.tokensUsed / 1_000_000) * tokenRate)}
                    </td>
                  </tr>
                ))}
            </tbody>
