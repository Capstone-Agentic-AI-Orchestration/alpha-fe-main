  ...
  const filteredRuntimes = useMemo(() => {
    let list = runtimes.filter(r => {
      ...
      if (searchQuery.trim()) {
        ...
        const matchesModels = r.modelsLoaded.some(m => m.toLowerCase().includes(q));
    ...
    list = [...list].sort((a, b) => {
      ...
      else if (sortBy === 'models') cmp = a.modelsLoaded.length - b.modelsLoaded.length;
      else cmp = a.latencyMs - b.latencyMs; // default latency
            ...
            return (
              ...
              >
                ...
                <div className="col-span-2 truncate flex items-center gap-1.5">
                  <span className="font-mono text-white text-[11px]">{rt.modelsLoaded.length} models</span>
                  <span className="text-gray-500 font-mono text-[10px] truncate">
                    ({rt.modelsLoaded[0]?.split(':')[0] || 'none'})
                  </span>
                </div>

                {/* 5. Latency */}
                <div className="col-span-1 text-left font-mono text-xs">
                  <span className={rt.latencyMs < 50 ? 'text-emerald-400' : 'text-gray-400'}>
                    {rt.latencyMs}ms
                  </span>
                </div>
            ...
            <div className="space-y-2.5">
              <div className="flex items-center justify-between text-[11px] font-medium text-gray-400 uppercase tracking-wider">
                <span className="flex items-center gap-1.5">
                  ...
                  <span>Available Model Weights ({selectedRuntime.modelsLoaded.length})</span>
                </span>
                <span className="text-gray-500 font-mono">Ready for dispatch</span>
              </div>

              <div className="space-y-2">
                {selectedRuntime.modelsLoaded.map((model) => (
                  <div
                    key={model}
